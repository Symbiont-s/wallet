...
class ExpiringDelegations extends React.Component {
        ...
        switch (field) {
            case AMOUNT:
                if (sortMethod === ASC) {
                    data.sort(
                        (a, b) =>
                            parseFloat(a.vesting_shares.amount) -
                            parseFloat(b.vesting_shares.amount)
                    );
                } else {
                    data.sort(
                        (a, b) =>
    ...
    render() {
        ...
        delegation_log = delegation_log.map(item => {
            const currentVest =
                item.vesting_shares.amount /
                10 ** item.vesting_shares.precision;
            const vestsAsSteem = convertVestsToSteem(parseFloat(currentVest));
            return (
                <tr key={`${item.delegator}--${item.id}--${item.expiration}`}>
                    <td className="text--capitalize">
                        <span
                            onClick={() => {
                                updateClipboard(item.expiration);
                            }}
                    ...
                    {expiringVestingDelegationsPending ? (
                        ...
                        <div>
                            <table>
                                <thead>
                                    <tr>
                                        ...
                                        >
                                            {tt(
                                                'expiringdelegations_jsx.end_date'
                                            )}
                                            <span
                                                className={
                                                    sortBy === DATE
                                        ...
                                        <th
                                            onClick={() => {
                                                this.sortData(AMOUNT);
                                            }}
                                            style={{ cursor: 'pointer' }}
                                            className="column-number"
                                        >
                                            {tt(
                                                'expiringdelegations_jsx.amount_returned'
                                            )}
                                            <span
                                                className={
                                                    sortBy === AMOUNT
                            ...
                            {expiringVestingDelegations.length > PAGE_LIMIT ? (
                                <Pagination
                                    totalRecords={
                                        expiringVestingDelegations.length
                                    }
                        ...
                        <table>
                            <tbody>
                                <tr>
                                    <td style={{ textAlign: 'center' }}>
                                        {tt(
                                            'expiringdelegations_jsx.no_delegations'
                                        )}
                                    </td>
                                </tr>
                            </tbody>
